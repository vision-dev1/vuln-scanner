...
# Try to import requests, if not available provide instructions
...
try:
    ...
    print("")
    # We'll define a simple fallback for demonstration purposes
    class MockRequestsResponse:
        def __init__(self, status_code=200, text="", headers=None):
            ...
            self.cookies = []
            self.url = "https://example.com"  # Add url attribute
    
    class MockRequests:
        def get(self, url, **kwargs):
            # Simple mock that returns a successful response
...
class VulnerabilityScanner:
    ...
    def check_cookie_flags(self, url):
        ...
        try:
            ...
            for cookie in cookies:
                ...
                # Check for HttpOnly flag (using get because some Cookie implementations may not have httponly)
    ...
    def check_server_banner(self, url):
        """Check server banner fingerprinting"""
        try:
            parsed = urlparse(url)
            hostname = parsed.hostname
...
def start_server():
    ...
    # Change to the directory where frontend files are located
